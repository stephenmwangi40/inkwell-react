...
export default function CustomerMessages({ orders, customers, messages, selectedOrder, setSelectedOrder, onSend }) {
        ...
        {selectedOrder ? (
          <>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {thread.map((m) => (
                <div key={m.id} className={`max-w-[85%] rounded-xl px-3.5 py-2.5 text-sm ${m.from === "customer" ? "ml-auto bg-blue text-white" : "bg-slate-tint text-ink"}`}>
                  <p>{m.text}</p>
                  <div className={`mt-1 text-[0.7rem] ${m.from === "customer" ? "text-white/70" : "text-slate"}`}>{m.time}</div>
                </div>
              ))}
            ...
            <div className="flex flex-col gap-2 border-t border-line p-3 sm:flex-row">
              <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (onSend(selectedOrder, text), setText(""))} placeholder="Write a message…" className="flex-1 rounded-[10px] border border-line px-3 py-2 text-sm outline-none focus:border-blue" />
